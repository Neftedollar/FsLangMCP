...
`v0.18.0` ships **35 MCP tools** across headline, semantic, LSP-proxy, and meta categories. The surface is still evolving toward a stable shape: v0.11.0 consolidated the former search/check aliases behind `find` and `check`, v0.15.0 made incomplete evidence explicit and tightened typed tool input to strict JSON, v0.16.0 made payloads self-explaining, and v0.17.0 hardened the same surface around result limits, admission-aware deadlines for find/check/test sweeps, test/outline coverage, stale runtime detection, and dependency closure. The v0.17.1 patch added declared-member coverage, end-to-end find deadlines, an actual serialized-response ceiling, and waiter-aware discovery. Version 0.18.0 adds query/snapshot-bound `find` cursors (#259), short-lived MSBuild evaluation helpers (#150), explicit snippet placement (#196), and compiled backtick-name lookup (#267). Old `find` cursors require restarting the search after upgrading; other tools keep their cursor formats. A 1.0 release will lock the contract.
