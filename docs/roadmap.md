...
`v0.15.0` ships **35 MCP tools** across headline, semantic, LSP-proxy, and meta categories. The surface is still evolving toward a stable shape: v0.13.1 consolidated the former search/check aliases behind `find` and `check`, while v0.15.0 made incomplete evidence explicit and tightened typed tool input to strict JSON. A 1.0 release will lock the contract.
